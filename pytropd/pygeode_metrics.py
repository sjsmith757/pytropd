...
def metrics_dataset(dataset: pyg.Dataset, metric: str, **params) -> pyg.Dataset:

    """Return a dataset of the metrics"""

    metric_property_name = dict(
        edj=["u", "Zonal wind"],
    ...
    dataset, _ = extract_property(
        dataset, property_name=["lat", "Latitude"], p_axis_status=p_axis_status
    )
    dataset, found_pres = extract_property(
        dataset, property_name=["pres", "Pressure"], p_axis_status=p_axis_status
    )
    ...
    var, _ = extract_property(
        dataset, property_name=metric_property_name[metric], p_axis_status=p_axis_status
    )
...
def metric_var(
    ...
    **params
    ...
    if len(output) < 1:
        raise ValueError(
            "No valid outputs are requested from metric calculation. Possible outputs are %s."
            % str(ovars)
        )
    ...
    if X.hasaxis("Lat"):
        ...
        if X.hasaxis("Pres") and (p_axis_status == 1):
            ...
            out_order = [
                i
                for i in range(len(inaxes))
                if i not in [pres_axis_index, lat_axis_index]
            ]
    ...
    for outsl, (xdata,) in loopover([X], oview, inaxes, pbar=pbar):
        ...
        if method_used == "fit":
            metric_lat[outsl], metric_value[outsl] = metric_function(
                xdata, lat_values, **params
            )
        else:
            metric_lat[outsl] = np.squeeze(
                np.array(metric_function(xdata, lat_values, **params))
            )
    ...
    if "lat" in output:
        ...
        metric_lat = Var(
            oaxes, values=metric_lat, name=hem + "_metric_lat", atts=lat_attrs
        )
...
def extract_property(
    dataset: pyg.Dataset, property_name: List[str], p_axis_status: int
    ...
    if property_name[0] == "pres":
        ...
        if p_axis_status and len(index) == 0:
            print(dataset)
            raise KeyError(
                "%s not found in Dataset. Valid variable names are %s"
                % (property_name[1], property_name_list)
            )

        elif len(index) == 1:
            # if we find a pres axis convert it to a pyg.Pres axis and return data
            print(
                "Using %s in the dataset as the %s"
                % (dataset_keys[index[0]], property_name[1])
            )
            pres_axis = getattr(dataset, dataset_keys[index[0]])
            print(
                "Replacing pyg.NamedAxis %s with a pyg.Pres axis in the dataset"
                % (dataset_keys[index[0]])
            )
            return (
                dataset.replace_axes({dataset_keys[index[0]]: pyg.Pres(pres_axis[:])}),
                1,
            )
    ...
    if len(index) > 1:
        print(dataset)
        print(
            "More than one possible key for %s found. Valid variable names are %s"
            % (property_name[1], property_name_list)
        )
        raise KeyError
    ...
    if property_name[0] == "lat" and find_index:
        ...
        print(
            "Replacing pyg.NamedAxis %s with a pyg.Lat axis in the dataset"
            % (dataset_keys[index[0]])
        )
...
def chop_by_hemisphere(dataset: pyg.Dataset, hem: str = "nh") -> pyg.Dataset:
