...
def TropD_Metric_EDJ(U, lat, lev=None, method='peak',
                     n_fit=1, **maxlat_kwargs):
    ''' 
    ...
    if U_grid_shape != input_grid_shape:
        raise ValueError(f'last axes of U w/ shape {U_grid_shape},'
                         ' not aligned with input grid of shape '+
                         str(input_grid_shape))
    ...
    if method != 'fit': #max or peak
        ...
        PhiNH = TropD_Calculate_MaxLat(u850[...,NHmask], lat[NHmask],
                                       **maxlat_kwargs)
        PhiSH = TropD_Calculate_MaxLat(u850[...,SHmask], lat[SHmask],
                                       **maxlat_kwargs)
        ...
        for hem_mask in [NHmask, SHmask]:
            ...
            for i, Uh in enumerate(u_flat[:,hemmask]):
                m = np.nanmax(Uh)
                Im = np.nanargmax(Uh)
                ...
                if (n_fit > Im or n_fit > Uh.size-Im+1):
                    ...
                    N = n_fit
                p = polynomial.polyfit(lath[Im-N:Im+N+1], 
                                       Uh[Im-N:Im+N+1], deg=2)
...
def TropD_Metric_OLR(olr, lat, method='250W', Cutoff=None, **maxlat_kwargs):
    ...
    if olr.shape[-1] != lat.size:
        raise ValueError(f'last axis of olr had shape {olr.shape[-1]},'
    ...
    olr_max_lat_NH = TropD_Calculate_MaxLat(olr[...,NH_subpolar_mask],
                                            lat[NH_subpolar_mask],
                                            **maxlat_kwargs)
    olr_max_lat_SH = TropD_Calculate_MaxLat(olr[...,SH_subpolar_mask], 
                                            lat[SH_subpolar_mask],
                                            **maxlat_kwargs)
    ...
    if method in ['cutoff','250W', '20W','10Perc']:
        ...
        if method == '250W':
            ...
            NHCutoff = olr_max_NH - 20.
            SHCutoff = olr_max_SH - 20.
        elif method == '10Perc':
        ...
        PhiSH = TropD_Calculate_ZeroCrossing(olr_SH[...,::-1] - SHCutoff,
                                             lat[SHmask][::-1])
...
def TropD_Metric_PE(pe,lat,method='zero_crossing',lat_uncertainty=0.0):
    ...
    if pe.shape[-1] != lat.size:
        raise ValueError(f'last axis of P-E had shape {pe.shape[-1]},'
                          f' not aligned with lat size {lat.size}')
    ...
    pe_grad = interp1d(lat_mid, dpedy, axis=-1, bounds_error=False,
                    fill_value=(dpedy[...,0],dpedy[...,-1]))(lat)
    ...
    Emax_latNH = TropD_Calculate_MaxLat(-pe[...,NH_subpolar_mask], 
                                        lat[NH_subpolar_mask], n=30)
    Emax_latSH = TropD_Calculate_MaxLat(-pe[...,SH_subpolar_mask],
                                        lat[SH_subpolar_mask], n=30)
    ...
    ZC1_latNH = TropD_Calculate_ZeroCrossing(peNH_after_Emax, latNH,
                                        lat_uncertainty=lat_uncertainty)
    ZC1_latSH = TropD_Calculate_ZeroCrossing(peSH_after_Emax[...,::-1],
                                                latSH[::-1], lat_uncertainty=
                                                    lat_uncertainty)
    ...
    for i, ipe_grad in enumerate(pe_grad_flat):
        ...
        peNH_increases_at_ZC[i_unrav] = (interp_pe_grad(
                                                ZC1_latNH.flatten()[i]) > 0)
        peSH_increases_at_ZC[i_unrav] = (interp_pe_grad(
                                                ZC1_latSH.flatten()[i]) < 0)
    ...
    ZC2_latNH = TropD_Calculate_ZeroCrossing(peNH_after_ZC, latNH,
                                            lat_uncertainty=lat_uncertainty)
    ZC2_latSH = TropD_Calculate_ZeroCrossing(peSH_after_ZC, latSH,
                                            lat_uncertainty=lat_uncertainty)
...
def TropD_Metric_PSI(V, lat, lev, method='Psi_500',
                     lat_uncertainty=0, patch_10perc=False):
    ''' 
    ...
    if V.shape[-2:] != (lat.size,lev.size):
        raise ValueError(f'final dimensions on V {V.shape[-2:]} and grid '
    ...
    if (method == 'Psi_500') or (method == 'Psi_500_10Perc'):
        ...
        layer_700_to_300 = (lev <= 700.) & (lev >= 300.)
        P = np.trapz(Psi[...,layer_700_to_300]*cos_lat, 
                     lev[layer_700_to_300]*100., axis=-1)
    elif method == 'Psi_500_Int':
        # Use integrated Psi from p=0 to level nearest to 500 hPa
        PPsi = cumtrapz(Psi*cos_lat, 100.*lev, axis=-1, initial=0.)
        P = PPsi[...,find_nearest(lev, 500.)]
    elif method == 'Psi_Int':
        # Use vertical mean of Psi 
        P = np.trapz(Psi*cos_lat, 100.*lev, axis=-1)
    else:
        raise ValueError('unrecognized method ', method)
    ...
    Pmax_latNH = TropD_Calculate_MaxLat(P[...,NH_subpolar_mask],
                                        lat[NH_subpolar_mask])
    Pmax_latSH = TropD_Calculate_MaxLat(-P[...,SH_subpolar_mask],
                                        lat[SH_subpolar_mask])
    ...
    if method == 'Psi_500_10Perc':
        ...
        if patch_10perc:
            PhiSH = TropD_Calculate_ZeroCrossing(PSH_in_between[...,::-1] 
                                                    - 0.1*PminSH, latSH[::-1])
        else:
            PhiSH = TropD_Calculate_ZeroCrossing(PSH_in_between[...,::-1] 
                                                    + 0.1*PminSH, latSH[::-1])
    else:
        PhiNH = TropD_Calculate_ZeroCrossing(PNH_in_between, latNH,
                                             lat_uncertainty=lat_uncertainty)
        PhiSH = TropD_Calculate_ZeroCrossing(PSH_in_between[...,::-1],
                                             latSH[::-1],
                                             lat_uncertainty=lat_uncertainty)
...
def TropD_Metric_PSL(ps, lat, method='peak', **maxlat_kwargs):
    ...
    if ps.shape[-1] != lat.size:
        raise ValueError(f'last axis of ps had shape {ps.shape[-1]},'
                          f' not aligned with lat size {lat.size}')
...
def TropD_Metric_STJ(U, lat, lev, method='adjusted_peak', **maxlat_kwargs):
    ...
    if U.shape[-2:] != (lat.size,lev.size):
        raise ValueError(f'last axes of U had shape {U.shape[-2:]},'
...
def TropD_Metric_TPB(T, lat, lev, method='max_gradient',
                     Z=None, Cutoff=1.5e4, trop_kwargs={}, **maxlat_kwargs):
    ''' 
    ...
    if T.shape[-2:] != (lat.size,lev.size):
        raise ValueError(f'final dimensions on T {T.shape[-2:]} and grid '
                           f'coordinates don\'t match ({lat.size},{lev.size})')
    ...
    if 'max_' in method: #'max_gradient' or 'max_potemp'
        ...
        PhiNH = TropD_Calculate_MaxLat(F[...,NHmask], lat[NHmask],
                                      **maxlat_kwargs)
        PhiSH = TropD_Calculate_MaxLat(F[...,SHmask], lat[SHmask],
                                       **maxlat_kwargs)
        ...
        PhiNH = TropD_Calculate_ZeroCrossing(Ht[...,NHmask] - Cutoff, 
                                             lat[NHmask])
        PhiSH = TropD_Calculate_ZeroCrossing(Ht[...,SHmask][...,::-1] - Cutoff,
                                             lat[SHmask][::-1])

    return PhiSH, PhiNH
  
def TropD_Metric_UAS(U, lat, lev=None,
                     method='zero_crossing', lat_uncertainty = 0):
    ''' 
    ...
    uas_min_lat_NH = TropD_Calculate_MaxLat(-uas[...,NH_subpolar_mask],
                                            lat[NH_subpolar_mask])
    uas_min_lat_SH = TropD_Calculate_MaxLat(-uas[...,SH_subpolar_mask],
                                            lat[SH_subpolar_mask])
    ...
    PhiNH = TropD_Calculate_ZeroCrossing(uNH_after_Umin, lat[NHmask],
                                         lat_uncertainty=lat_uncertainty)
    PhiSH = TropD_Calculate_ZeroCrossing(uSH_after_Umin[...,::-1], 
                                         lat[SHmask][::-1],
                                         lat_uncertainty=lat_uncertainty)
