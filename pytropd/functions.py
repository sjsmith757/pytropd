...
def find_nearest(array, value, axis=None, skipna=False):
    ...
    if (value < array.min()) | (value > array.max()):
        warnings.warn('searching outside the bounds of an array will return '
                      'index of the closest boundary', category=RuntimeWarning,
                      stacklevel=2)
    if not skipna:
        if np.isnan(array).any():
            warnings.warn('searching an array with NaNs will return index '
...
def TropD_Calculate_MaxLat(F,lat,n=6,axis=-1):
    ...
    if not np.isfinite(F).any():
        raise ValueError('input field F has only NaN/inf values,'
                         ' max lat cannot be computed')
    if not F.shape[axis] == lat.size:
        raise ValueError(f'input field lat axis of size {F.shape[axis]} not '
    ...
    if not np.isfinite(F).all():
        F_filled = np.where(np.isfinite(F),F,0.)
        #find edges of nan regions
        nanbounds = np.diff(np.isfinite(F),axis=-1)
        #ensure we only have one contiguous region
        extra_nans_check = (nanbounds.sum(axis=-1)>2.).any()
        interior_nans_check = (np.isfinite(F)[...,0]
                                & np.isfinite(F)[...,-1]).any()
        if extra_nans_check or interior_nans_check:
            warnings.warn('detected NaN/inf data located between valid data, '
                          'this may not be handled correctly',  stacklevel=2,
        ...
        rbounds = np.where(nanbounds & np.isfinite(F[...,1:]), F[...,1:], 0.)
        lbounds = np.where(nanbounds & np.isfinite(F[...,:-1]), F[...,:-1], 0.)
        bounds = (np.concatenate([pad, rbounds], axis=-1) 
                  + np.concatenate([lbounds, pad], axis=-1))
        #now integrate and remove the extra boundary values added by trapz
        nom = (np.trapz(F_filled**n*lat, lat, axis=-1) 
               - np.trapz(bounds**n*lat, lat, axis=-1)/2.)
        denom = (np.trapz(F_filled**n, lat, axis=-1) 
                 - np.trapz(bounds**n, lat, axis=-1)/2.)
        ...
        #weighted integral to account for discrete grid
        Ymax = (np.trapz((F**n)*lat, lat, axis=-1) 
                / np.trapz(F**n, lat, axis=-1))

    return Ymax

def TropD_Calculate_Mon2Season(Fm, season, m=None, first_jan_idx=0,
                                patch_indexing=True, axis=0):
    ''' 
    ...
    if m is not None:
        warnings.warn('use of parameter m is deprecated and will be removed in'
    ...
    # values (23 months only). errors also result when setting first_jan_idx
        ...
        last_jan_idx = (Fm.shape[0]-first_jan_idx+1 
                        - (Fm.shape[0]-first_jan_idx+1)%12)
...
def TropD_Calculate_StreamFunction(V, lat, lev):
    ...
    psi =  (cumtrapz(V, lev*100., axis=-1, initial=0.)
            * (EARTH_RADIUS/GRAV*2.*np.pi) * cos_lat)
...
def TropD_Calculate_TropopauseHeight(T ,P, Z=None,
    ...
    GI = interpG(PI)
    interpT = interp1d(Pk_mid, T_mid, kind='linear', axis=-1,
                    fill_value='extrapolate')
    TI = interpT(PI)
    # points in upper troposphere where lapse rate is less then 2km
    trop_layer = (GI<=2.) & (PI<(550.*100.)**KAPPA) & (PI>(75.*100.)**KAPPA)
...
def TropD_Calculate_ZeroCrossing(F, lat, lat_uncertainty=0.0,
    ...
    if F.shape[axis] != lat.size:
        raise ValueError(f'input array F with lat axis of size {F.shape[axis]}'
                         f' is not aligned with lat coords of size {lat.size}')
    if lat.size < 4:
        raise ValueError('requires at least 4 latitudes to find zero '
                         f'crossing, got {lat.size}')
    ...
    for i in range(ZC.size):
        ...
        if np.abs(Di[a1])==1:
            ...
            ZC[iband] = (-Fi[a1] * (lat[a1+1]-lat[a1])
                                                / (Fi[a1+1]-Fi[a1]) + lat[a1])
